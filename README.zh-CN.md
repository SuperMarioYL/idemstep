...
- **代理**——为每个带 `x-idem-key` 的出站请求计算 `requestSig`（`method + host + path + body-hash`）。若某个 `committed` 的键已经拥有该签名，则回放缓存响应而不转发。重试永远到不了上游站点。
- **`IdemStore`**——默认在内存中；给 CLI 传 `--store path.json`（或 `new IdemStore({ filePath })`）即可让去重状态在进程重启后留存。Redis/Postgres 不在 v0.1 范围内。
...
| `startProxy` | `startProxy(opts?) => RunningProxy` | 启动本地拦截/去重代理。 |
| `IdemStore` | `new IdemStore({ filePath? })` | 键 → `StepRecord` 的存储（内存或 JSON 文件）。 |
